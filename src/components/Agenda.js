...
export default function Agenda({ pedidos, setPedidos, title, orderType, shop }) {
    ...
    const toggleCompleted = function (id) {
        ...
        axios.post('https://server.floreseva.com/api/actualizar', newPedidos[index])
            .then(response => {
    ...
    function editarPedido(event) {
        ...
        if (newPedidos[index].deliveryDate) {
            setPedidos(newPedidos)
            axios.post('https://server.floreseva.com/api/editar', newPedidos[index])
                .then(response => {
                    console.log(response);
                })
                .catch(err => {
    ...
    const deleteOrder = (event) => {
        ...
        if (confirm) {
            ...
            setPedidos(newPedidos);
            // Realizamos el POST a la base de datos
            axios.post('https://server.floreseva.com/api/borrar', newPedidos[index])
                .then(response => {
                    console.log(response);
                })
                .catch(err => {
    ...
    useEffect(() => {
        let isMounted = true;
        fetch('https://server.floreseva.com/api/orders')
            .then(res => res.json())
            .then(data => {
                if (isMounted) {
                    setPedidos(data)
